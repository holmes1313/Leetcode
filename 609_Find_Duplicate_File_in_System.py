# -*- coding: utf-8 -*-
"""
Created on Wed Nov 13 10:56:21 2019
...
all the file paths of the files that have the same content. A file path is a string that has the following format:
...
class Solution(object):
    def findDuplicate(self, paths):
        ...
        for path in paths:
            ...
            files = path_split[1:]
            for f in files:
                content = f[f.find('(') + 1: f.find(')')]   # extract content in parenthese
                result[content].append(direct + '/' + f[:f.find('(')])

        return [v for v in result.values() if len(v) > 1 ]   # filter for a list
